...
function IsAdmissionOfRegistration(arr) {

    for (let i = 0; i < arr.length; i++) {
        if (arr[i].classList.contains('color_red')) {
            return false;
        }
    }
...
const SaveInputValuesInLocalStorage = () => {
    let arrStateInputs = document.querySelectorAll('.item_footer_text');

    if (IsAdmissionOfRegistration(arrStateInputs)) {
        let valuesInputs = {
            name: inptName.value,
            lastName: inptLastName.value,
            login: inptLogin.value,
            eMail: inptEmail.value,
            pass: inptPass.value
        };

        localStorage.setItem("valuesInputs", JSON.stringify(valuesInputs));
    } else {
...
window.onload = () => {
    let valuesInputs = JSON.parse(localStorage.getItem("valuesInputs"));

    if (valuesInputs) {
        inptName.value = valuesInputs.name;
        inptLastName.value = valuesInputs.LastName;
        inptLogin.value = valuesInputs.login;
        inptEmail.value = valuesInputs.eMail;
        inptPass.value = valuesInputs.pass;
    }
}